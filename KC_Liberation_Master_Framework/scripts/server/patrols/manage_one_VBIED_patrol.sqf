...
while { GRLIB_endgame == 0 } do {
      _spawnsector = "";
         ...
         if ( count _usable_sectors > 0 ) then {
             ...
             waitUntil {
                 sleep 10;
                 ((({alive _x} count (units _grp)) == 0) || (count ([getpos leader _grp, 1500] call KPLIB_fnc_getNearbyPlayers) == 0) || (count ([getpos leader _grp, 300] call KPLIB_fnc_getNearbyPlayers) > 1))
             };

             _nearestPlayer = ([markerPos _grp, 400] call KPLIB_fnc_getNearbyPlayers) select 0;
             if !(isNil _nearestPlayer) then {
                while {(count (waypoints _grp)) != 0} do {deleteWaypoint ((waypoints _grp) select 0);};
                (units _grp joinSilent (createGroup east));
                _civveh setVehicleLock "LOCKED";
                _nearestPlayer = ([markerPos _grp, 500] call KPLIB_fnc_getNearbyPlayers) select 0;
                while {!(isNil _nearestPlayer) && (_nearestPlayer distance2D _grp) > 5} do {
                  _grp setspeedmode "FULL";
                  _grp move position _nearestPlayer;
                  sleep 1;
                };
                _bomb = "Bo_GBU12_LGB_MI10" createVehicle (position _civveh);
                _bomb setDamage 1;
                deleteVehicle _bomb;
                _civveh setDamage 1;
