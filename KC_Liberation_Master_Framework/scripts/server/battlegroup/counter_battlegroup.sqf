...
private _target_pos = "";
while {GRLIB_csat_aggressivity >= 0.75 && GRLIB_endgame == 0} do {		// reduced aggressiveness threshold from 90% to 75%
    _sleeptime = (600 + (random 900)) / (([] call KPLIB_fnc_getOpforFactor) * GRLIB_csat_aggressivity);	//When aggressiveness is above 75%, only sleep 600 (10 mins)+ up to 15 mins (10-25 mins max)	orignal 30-60mins
    ...
    {
        if (
            (armor_weight >= 50 && {(objectParent _x) isKindOf "Tank"})	//unmodified I believe this is to pick a blufor to target the counter attack toward (to find a sector)
